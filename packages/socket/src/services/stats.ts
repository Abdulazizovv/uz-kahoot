...
import { getKahootSessionResult, listKahootSessionIds } from "@eduarena/socket/services/kahoot-results-store"
import {
...
const withinRange = (iso: string, fromD: dayjs.Dayjs | null, toD: dayjs.Dayjs | null) => {
  const d = dayjs(iso)
...
export const computeGroupStats = (payload: { groupId: string; from?: string; to?: string }): GroupQuizStats => {
  ...
  for (const [testId, title] of tfIndex.entries()) {
    const attempts = getTrueFalseResults(testId)
      ...
      .filter((a) => withinRange(a.submittedAt, fromD, toD))
  ...
  // Kahoot results (not yet persisted in this build) -> keep empty
  for (const sessionId of listKahootSessionIds()) {
    const session = getKahootSessionResult(sessionId)

    if (!session) {continue}

    if (!withinRange(session.finishedAt, fromD, toD)) {continue}
  ...
  const avgScorePct =
    attempts === 0 ? 0 : clampPct((summaries.reduce((acc, s) => acc + (s.total ? (s.score / s.total) * 100 : 0), 0) / attempts))
  ...
  for (const kind of ["truefalse", "matching", "kahoot"] as const) {
    ...
    byKind[kind] = {
      ...
      avgScorePct:
        list.length === 0
          ? 0
          : clampPct(list.reduce((acc, s) => acc + (s.total ? (s.score / s.total) * 100 : 0), 0) / list.length),
    }
  }

  const testsMap = new Map<string, GroupQuizStats["tests"][number] & { studentKeys: Set<string> }>()
  ...
  return {
    ...
    byKind: {
      truefalse: { attempts: byKind.truefalse.attempts, avgScorePct: round1(byKind.truefalse.avgScorePct) },
      matching: { attempts: byKind.matching.attempts, avgScorePct: round1(byKind.matching.avgScorePct) },
      kahoot: { attempts: byKind.kahoot.attempts, avgScorePct: round1(byKind.kahoot.avgScorePct) },
    },
    tests: tests.map((t) => ({ ...t, avgScorePct: round1(t.avgScorePct) })),
    topStudents: topStudents.map((s) => ({ ...s, avgScorePct: round1(s.avgScorePct), bestScorePct: round1(s.bestScorePct) })),
    recentAttempts,
  }
}

export const computeTestStats = (payload: { kind: QuizKind; testId: string }): QuizTestStats | null => {
