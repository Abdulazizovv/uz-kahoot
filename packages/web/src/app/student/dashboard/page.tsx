...
export default function StudentDashboard() {
  ...
  const readCount = itPosts.filter((post) => post.status === "read").length
  ...
  const posts = itPosts.slice(0, 5).map((post) => ({
    id: post.id,
    title: post.title,
    progress: post.progress,
    status: post.status === "read" ? ("done" as const) : ("new" as const),
  }))

  const recentGames = [
    { id: 1, name: "Frontend asoslari testi", date: "2 soat oldin", score: 95, rank: 3 },
    { id: 2, name: "JavaScript praktikum", date: "1 kun oldin", score: 87, rank: 8 },
    { id: 3, name: "SQL qisqa test", date: "2 kun oldin", score: 92, rank: 5 },
  ]
          ...
          <div className="mt-8 grid gap-4 lg:grid-cols-3 lg:gap-6">
              ...
              <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-600">
                      O'qishni davom ettirish yoki yangisini boshlash.
                    </p>
                  </div>
                  <Link
                    href="/student/labs"
                ...
                <div className="mt-5 space-y-3">
                  {posts.map((post) => {
                    const actionLabel = post.status === "done" ? "Ko'rish" : "Boshlash"
                    const barColor =
                      post.progress === 100
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <div className="mt-5 space-y-3">
                  {[
                    { title: "Postlar izlovchi", desc: "3 ta post o'qildi", tone: "border-amber-200 bg-amber-50 text-amber-900" },
