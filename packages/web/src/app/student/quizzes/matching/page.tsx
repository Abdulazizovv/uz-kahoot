...
import { MatchingTestSummary } from "@eduarena/common/types/matching"
import Link from "next/link"
import { useEffect, useMemo, useState } from "react"
import toast from "react-hot-toast"

export default function StudentMatchingListPage() {
    ...
    ;(async () => {
      ...
      try {
        const list = await apiGet<MatchingTestSummary[]>(
          `/api/async/matching/tests?mode=student&groupId=${encodeURIComponent(groupId)}`,
        )
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="mb-6">
            ...
            <p className="mt-1 text-slate-600">
              Chapdagi rasm/matnni o&apos;ngdagi javob bilan moslashtiring.
            </p>
          </div>
          ...
          {loadingProfile ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {sorted.map((t) => (
                <Link
                  key={t.id}
                  href={`/student/quizzes/matching/${t.id}`}
